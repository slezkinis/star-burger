...
Для запука проекта на сервере нужно указать настройки в файле `.env`. Затем настроить `nginx` и создать процесс `star-burger`. После этого запустить скрипт `deploy_star_burger.sh`
