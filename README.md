...
Для запука проекта на сервере нужно указать настройки в файле `.env`. Затем настроить `nginx` и создать процесс `star-burger`. После этого запустить скрипт `deploy_star_burger.sh`, указав через пробел токен от ROLLBAR и имя проекта в ROLLBAR.

## Как выполнить деплой
После залива изменений на GitHub просто запустите скрипт `deploy_star_burger.sh`, указав через пробел токен от ROLLBAR и имя проекта в ROLLBAR  и он всё сделает за Вас:
```sh
./deploy_star_burger.sh TOKEN FirstProject
```
